...
import { ArrowLeft, ArrowRight, Check, ChevronDown, CircleHelp, Footprints, Flame, Leaf, Loader2, Plus, Settings, Sparkles, Trash2, Utensils } from "lucide-react";
...
const Home = () => {
  ...
  return (
    <div className="shell">
      <aside className="sidebar"><div className="brand"><span className="brand-mark"><Leaf size={18} /></span><span>AJX90<span className="brand-dot">.</span></span></div><nav><button className="nav-active" data-testid="dashboard-navigation"><Utensils size={18} />Daily dashboard</button><button data-testid="history-navigation"><Flame size={18} />Meal history</button></nav><div className="side-note"><div className="note-icon"><Sparkles size={16} /></div><p><strong>Good food, clear data.</strong><br />Paste what you ate and let your day take shape.</p></div><div className="help"><CircleHelp size={16} />Nutrition estimates are a guide</div></aside>
      <main className="main"><header className="topbar"><div><p className="eyebrow">YOUR NUTRITION, SIMPLIFIED</p><h1>Daily dashboard</h1></div><div className="date-picker"><button data-testid="previous-day-button" onClick={() => setDate(subDays(date, 1))}><ArrowLeft size={17} /></button><span data-testid="selected-date">{format(date, "EEEE, MMM d")}{dateKey === format(new Date(), "yyyy-MM-dd") && <em>Today</em>}</span><button data-testid="next-day-button" onClick={() => setDate(addDays(date, 1))}><ArrowRight size={17} /></button></div></header>
        ...
        {estimate && <section className="estimate" data-testid="nutrition-estimate-card"><div><p className="eyebrow">ESTIMATE READY · {segment.toUpperCase()}</p><h3>Here’s the nutrition read</h3><p className="estimate-note">{estimate.note || "Based on typical preparation and serving sizes."}</p><p className="edit-hint">Review or adjust any number before saving.</p></div><div className="estimate-grid">{[["Calories", estimate.calories, "kcal", "calories"], ["Protein", estimate.protein, "g", "protein"], ["Carbs", estimate.carbs, "g", "carbs"], ["Fibre", estimate.fibre, "g", "fibre"], ["Fats", estimate.fats, "g", "fats"]].map(([label, value, unit, field]) => <EstimateMetric key={label} label={label} value={value} unit={unit} onChange={(nextValue) => updateEstimate(field, nextValue)} testId={`edit-${field}-input`} />)}</div><button className="save" data-testid="save-meal-button" onClick={saveMeal} disabled={saving}><Check size={17} /> {saving ? "Saving..." : "Save to today"}</button></section>}
        <section className="walking-card" data-testid="walking-tracker-card"><div className="walking-head"><div className="walking-title"><span className="walk-icon"><Footprints size={19} /></span><div><p className="eyebrow">DAILY MOVEMENT</p><h3>Walking & deficit</h3></div></div><Settings size={17} className="settings-icon" /></div><div className="walking-inputs"><label><span>Steps walked</span><input data-testid="steps-input" type="number" min="0" value={steps} onChange={(e) => setSteps(e.target.value)} placeholder="e.g. 8000" /></label><label><span>Current weight</span><div className="unit-input"><input data-testid="weight-input" type="number" min="1" step="0.1" value={weight} onChange={(e) => setWeight(e.target.value)} placeholder="Enter kg" /><small>kg</small></div></label><button data-testid="save-walking-button" className="save-walking" onClick={saveWalking}>{weightSaved ? <Check size={16} /> : <Check size={16} />} {weightSaved ? "Saved" : "Save activity"}</button></div><div className="deficit-strip"><div><span>Walking burn</span><strong data-testid="walking-calories">+{walkingCalories} kcal</strong></div><div><span>Daily burn</span><strong>{totalBurn.toLocaleString()} kcal</strong></div><div className={deficit >= 0 ? "deficit-positive" : "deficit-negative"}><span>{deficit >= 0 ? "Today’s deficit" : "Over baseline"}</span><strong data-testid="daily-deficit">{deficit >= 0 ? "−" : "+"}{Math.abs(Math.round(deficit)).toLocaleString()} kcal</strong></div><div><span>Est. weight change</span><strong data-testid="estimated-weight-loss">{deficit >= 0 ? "−" : "+"}{estimatedLoss.toFixed(2)} kg</strong><small>{deficit >= 0 ? "based on 7,700 kcal / kg" : "estimated gain if repeated"}</small></div></div></section>
